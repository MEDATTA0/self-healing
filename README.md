...
> **New node?** If you are setting up a bare Fedora machine to join an existing cluster, run `node-setup/node-setup.sh`. It installs Docker Engine, enables the Docker and kubelet services, and joins the node to the cluster.

---

## Quick Start (Local)

Two scripts cover the full local workflow:

```bash
# 1. Install all dependencies and verify the build
./setup_integration.sh

# 2. Start the ML API and data collector in the background
./start_services.sh

# 3. (Optional) Smoke-test both services
./test_integration.sh
```

`setup_integration.sh` creates the Python virtual environment under `ml/.venv`, installs Python requirements, downloads Go module dependencies, and runs `go build ./...` to confirm the build is clean.
...
```bash
# Option A — creates the namespace automatically
bash prom-setup.k8s.sh

# Option B — step-by-step with namespace created separately
...
├── node-setup/
│   └── node-setup.sh           # Bootstrap Docker + kubelet on a Fedora node
